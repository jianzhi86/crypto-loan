...
const MARKETS = [
  ...
  { symbol: 'AVAX', name: 'Avalanche', icon: 'A', color: '#E84142', id: null,          supplyAPR: 4.1, borrowAPR: 7.2, maxLTV: 60, liqThresh: 70, liquidity: 'RM 840M',  totalBorrowed: 'RM 420M',  util: 50 },
...
const AVAX_PRICE  = { myr: 179,   usd: 38,   change24h: -1.5 };
...
export default function MarketsPage() {
  ...
  const getPrice = (m: typeof MARKETS[0]) => {
    ...
    if (m.symbol === 'AVAX')  return AVAX_PRICE;
  ...
  const totalTVL = 'RM 26.0B';
  const totalBor = 'RM 16.5B';
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Total Borrowed',     value: totalBor, sub: '63.5% utilisation',  c: '#eab308' },
            { label: 'Avg Borrow APR',     value: '6.0%',   sub: 'Weighted average',   c: '#ef4444' },
            { label: 'Avg Supply APR',     value: '3.1%',   sub: 'Weighted average',   c: '#22c55e' },
          ].map(s => (
              ...
              {displayed.map((m, i) => {
                ...
                return (
                  <tr key={m.symbol}
                    ...
                    {/* Actions */}
                    <td className="px-4 py-4">
                      <Link href="/" className="text-xs px-3 py-1.5 rounded-lg font-semibold text-white inline-block"
